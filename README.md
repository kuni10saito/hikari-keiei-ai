...
`agents/<会社キー>/<部門>.md` を置くと、その会社で部門AIが使えるようになる
（会社キーは `company_atsuta.md` → `atsuta`）。定義が無い会社では機能ごと無効。

| 学生の入力 | 動作 | API呼び出し |
|---|---|---|
| `@finance 原価率は？` | その部門の役割定義を system に足して1回 | 1回 |
| `@finance @quality @legal 〜` | **各部門を並列起動 → CEOが統合** | 部門数＋1回 |
| `@` 無し | 従来どおり | 1回 |

`MAX_AGENTS_PER_TURN`（既定4）で上限を置いている。増やすと費用が比例して増える。

**費用が現実的なのはプロンプトキャッシュのおかげ。** 部門呼び出しは同じ
`company_*.md` を system の先頭に置くので、2つ目以降はキャッシュ読出（0.1倍）
で済む。実測見込みは3部門＋統合で約9円、学生1人500円で54ターン。

部門呼び出しはテキストのみ（スキルもコード実行も渡さない、`max_tokens=2000`、
`effort=low`）。成果物を作れるのは統合側だけ。
